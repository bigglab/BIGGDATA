...
#Designed to analyze light chain sequences, in particular to build full length antibody sequences
#Based on AA sequence
#Requires modification Clonotyping address in Line 105
...
def VHPandas(fastq_files, groupname, CDR_list):
	...
	df_main=df_main[(df_main.Locus=='IGK')|(df_main.Locus=='IGL')]
	...
	df_collapsed=df_main[['Collapsed','CDR1_AA','CDR2_AA','CDR3_AA','VGene','JGene','V_AvgSHM','VIso','AA Seq']]
	...
	os.system("/home/gg-jm76798/Desktop/Lesson/Tools/CDR3_Clonotyping -f clustered.fasta > clusters.txt")
	...
	if search=='None':
		...
		df_main[['Clustered','Collapsed','ClusterID','CDR1_AA','CDR2_AA','CDR3_AA','VGene','JGene','V_AvgSHM','VIso']][df_main.Clustered>1].sort_values(['Clustered','ClusterID','Collapsed','CDR1_AA','CDR2_AA','CDR3_AA'],ascending=[False,True,False,True,True,True]).groupby(['ClusterID']).head(1).to_csv(clustered_AA_output, index=None, sep='\t')
		df_main[['Clustered','Collapsed','ClusterID','CDR1_AA','CDR2_AA','CDR3_AA','VGene','JGene','V_AvgSHM','VIso']][df_main.Clustered>1].sort_values(['Clustered','ClusterID','Collapsed','CDR1_AA','CDR2_AA','CDR3_AA'],ascending=[False,True,False,True,True,True]).drop_duplicates(['CDR1_AA','CDR2_AA','CDR3_AA','ClusterID']).to_csv(clustered_AA_raw_output, index=None, sep='\t')
		
		print 'Processing complete.\n'
		
		df_basics=df_main[['Clustered','Collapsed','ClusterID','CDR1_AA','CDR2_AA','CDR3_AA','VGene','JGene','V_AvgSHM','VIso','AA Seq']][df_main.Clustered>1].sort_values(['Clustered','ClusterID','Collapsed','CDR1_AA','CDR2_AA','CDR3_AA'],ascending=[False,True,False,True,True,True]).groupby(['ClusterID']).head(1)
		
		#Perform calculations
		VH_analysis.prepare_VL_only_analysis(numbers,df_basics,prefix,cluster)
		...
		for CDR in CDR_Search:
			...
			df_tiny=df_main[['Clustered','Collapsed','ClusterID','CDR1_AA','CDR2_AA','CDR3_AA','VGene','JGene','V_AvgSHM','VIso','AA Seq']][df_main.ClusterID==cluster].sort_values(['Clustered','ClusterID','Collapsed','CDR1_AA','CDR2_AA','CDR3_AA'],ascending=[False,True,False,True,True,True]).drop_duplicates(['AA Seq'])
...
#python VL_Analyzer_v1.04.py stitched.fastq -group 'x' -CDR {'None' or List-of-CDR3s}

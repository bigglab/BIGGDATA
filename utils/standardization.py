...
def build_annotation_dataframe_from_igfft_file(file_path, rmindels=True, append_ms_peptides=False, require_annotations=['aaSeqFR1', 'aaSeqCDR1', 'aaSeqFR2', 'aaSeqCDR2', 'aaSeqFR3', 'aaSeqCDR3', 'aaSeqFR4']):
    ...
    for k, df in df.groupby(np.arange(len(df)) // 10000):
        print("PROGRESS Parsing IGFFT to DB Format: {} annotations parsed, {}% done".format(k*chunk_size, round(k*chunk_size/float(full_length)*100, 2)))
        df = df.copy()
...
def build_annotation_dataframe_from_mixcr_file(file_path, rmindels=True, append_ms_peptides=False, require_annotations=['aaSeqCDR3']):
    ...
    print("{}: working on {} annotations in {} chunks".format(file_path, len(df_input), len(df_input)/chunk_size+1))
    for k, df in df.groupby(np.arange(len(df)) // 10000):
        print("PROGRESS Parsing MIXCR to DB Format: {} annotations, {}% done".format(k*chunk_size, round(k*chunk_size/float(full_length)*100, 2)))
        df = df.copy()
